...
describe("CheckboxField", () => {
  ...
  it("renders helper text and wires aria-describedby", () => {
    ...
    expect(screen.getByText("We will only send product announcements.")).toBeInTheDocument();
    expect(screen.getByRole("checkbox")).toHaveAttribute("aria-describedby", "updates-helper");
  });
  ...
  it("renders error text and marks the checkbox as invalid", () => {
    ...
    expect(screen.getByRole("checkbox")).toHaveAttribute("aria-describedby", "policy-error");
  });

  it("calls onChange when toggled", () => {
    const handleChange = vi.fn();
    render(<CheckboxField id="marketing" label="Receive marketing emails" onChange={handleChange} />);
