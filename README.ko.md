...
현재 프로젝트는 실제 백엔드 없이 동작하므로 `build:web:demo`가 `VITE_ENABLE_MSW=true`로 빌드되도록 했습니다.
...
실제 백엔드와 연결하는 배포로 전환할 때는 `build:web:demo`에서 `VITE_ENABLE_MSW=true`를 제거하고 API base URL을 별도로 설정합니다.
